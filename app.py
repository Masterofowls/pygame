...
# Pygame Initialization (Move all your game initialization code here)
pygame.init()
WIN = pygame.display.set_mode((1600, 900))  # Example size
...
class Player:
    ...
    def move(self, platforms, coins, enemies, door, mouse_x=None, mouse_y=None):
        if not self.alive:
        ...
        if mouse_x is not None and mouse_y is not None:
            # Basic mouse control: Player follows the mouse horizontally
            self.rect.x = mouse_x - self.rect.width // 2  # Center player on mouse
            # Add a jump if close enough to the player
            if abs(self.rect.centery - mouse_y) < 50 and self.jumps > 0:
                self.vel_y = -15
                self.jumps -= 1
                if self.jumps == 1:
                    self.can_double_jump = True
...
# Game State (Initialize your game objects here)
player_x = 100
player_y = 100
mouse_x = 0
mouse_y = 0
...
# ---------------------- Helper Functions -----------------------
def update_game_state(mouse_x, mouse_y):
    global player, platforms, coins, enemies, door
    player.move(platforms, coins, enemies, door, mouse_x, mouse_y)
...
def handle_mouse_event(json_str):
    global mouse_x, mouse_y
    mouse_data = json.loads(json_str)
    mouse_x = mouse_data['mouse_x']
    mouse_y = mouse_data['mouse_y']
    update_game_state(mouse_x, mouse_y)
...
if __name__ == '__main__':
    socketio.start_background_task(game_loop) # Start game loop in background
